...
public class Goal {
	...
	public void release() {
		...
		bindSet.release(); bindSet=null;
		for(Literal lit : clauses) {lit.release(); lit=null;}
		clauses.clear(); clauses=null;
		dataChoicePoints.release(); dataChoicePoints=null;
		ruleChoicePoints.release(); ruleChoicePoints=null;
	}
  ...
  //               have been bound.
	public void optimize() {
		...
		for(Literal lit : clauses)
			if(lit.isNegated()) {
				litVars.clear(); litVals.clear(); lit.getUniqueVariables(litVars, litVals, false);
		...
		for(Literal lit : clauses) {
			...
			if(!lit.isNegated()) {
				if(litVars.size() < minVars && !negLitSelected && 
					...
					seedLit = lit; minVars = litVars.size();
					for(String lvar : litVars)
						if(negVars.contains(lvar)) {negLitSelected=true;}
				}
			}
			//else return; // not sure how to optimize with negated literals Dec 3, 2015
		}
		if(seedLit == null) {System.out.println("no seed"); return;}
		...
		if(LbLits.size()>0) {
			for(Literal lit : LbLits) {
				clauses.add(lit);
				// update variables
				litVars.clear(); litVals.clear(); lit.getUniqueVariables(litVars, litVals, true);
		...
		while(oldClauses.size()>0) {
			litToAdd = null;
			if(clauses.size()==0) {litToAdd = seedLit;}
				...
				for(Literal lit : oldClauses) {
					...
					if(boundVals > 0) {
						if(systemPreds.contains(lit.getPredicate().getLabel())) {
							if(boundVars == litVars.size()) {litToAdd=lit; break;}
						}
						else {litToAdd=lit; break;}
					}
					if(boundVars > maxBoundVars) {
						if(systemPreds.contains(lit.getPredicate().getLabel())) {
							// for system predicates, all variables have to be bound
							if(boundVars == litVars.size()) {maxBoundVars=boundVars;	litToAdd=lit;}
						}
						else {maxBoundVars=boundVars;	litToAdd=lit;}
					}
					else if(maxBoundVars > 0 && boundVars == maxBoundVars && hiPriorityPreds.contains(lit.getPredicate().getLabel()))
						{maxBoundVars=boundVars; litToAdd=lit; break;}
				}
			}
			...
			// update variables
			litVars.clear(); litVals.clear(); litToAdd.getUniqueVariables(litVars, litVals, true);
			for(String lval : litVals) vals.add(lval);
		...
		// clean up
		oldClauses.clear(); oldClauses=null;
		vars.clear();       vars=null;
		litVars.clear();    litVars=null;
		vals.clear();       vals=null;
		litVals.clear();    litVals=null;
	}
