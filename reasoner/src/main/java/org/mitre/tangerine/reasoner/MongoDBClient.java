...
public class MongoDBClient {
	...
	public void release() {
		...
		//System.out.println("usage: " + invoked + " " + submitToMongo);
	}
	...
	public void getData(String sub, String pred, ArrayList<String> obj_lst) {
		...
		if(pred.equals("FreeTextSearch"))  {getData_fts(sub, pred, obj_lst); return;}
		if(pred.equals("RegexTextSearch")) {getData_regex(sub, pred, obj_lst); return;}

		if(!InstanceOfPredicates.contains(pred)) obj = obj_lst.get(0); else obj = "";
		...
		if(!sub.startsWith("?")) {
			...
			if(InstanceOfPredicates.contains(pred)) {getData_inst(obj_lst); return;}
			else {
				if(pred.startsWith("?")) {
					...
					else {getData_ems(sub, pred, obj); return;}
				}
			}
		}
		...
		if(getFullGraph && graphsToLoad.size()>0) {
			...
			gr_lst.clear(); gr_lst=null;
			gr_qry.clear(); gr_qry=null;
		}
	}
	...
	public void getData_fts(String sub, String pred, ArrayList<String> obj_lst) {
		...
		while(mongoCr.hasNext()) {
			...
			DBObject A = (DBObject) assrt.get("A"); A.removeField("P"); A.put("P", "FreeTextSearch");
			getAssertionFromJSONObject(assrt);
		}
		mongoCr.close();		
	}
	...
	public void getData_regex(String sub, String pred, ArrayList<String> obj_lst) {
		...
		while(mongoCr.hasNext()) {
			...
			DBObject A = (DBObject) assrt.get("A"); A.removeField("P"); A.put("P", "RegexTextSearch");
			getAssertionFromJSONObject(assrt);
		}
		mongoCr.close();
	}
