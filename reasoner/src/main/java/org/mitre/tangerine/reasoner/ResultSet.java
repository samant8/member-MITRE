...
public class ResultSet {
	...
	public void addTuple(Tuple tuple) {this.addTuple(tuple, null);}
		...
		if (!resmap.contains(ss)) {
			...
			if(displayOnAdd) {
				for (int j = 0; j < getVariables().size(); j++) {
					System.out.print(getVariables().get(j) + "=" + tuple.getCol()[j] + " ");
				}
				System.out.println();
				if(tuple_trace != null)
					for(String step : tuple_trace) System.out.println("\t"+step);
			}
		} else { // duplicate
			tuple = null;
		}
	...
	public void display() {
		for (int i = 0; i < getTuples().size(); i++) {
			...
			for (int j = 0; j < getVariables().size(); j++) {
				System.out.print(getVariables().get(j) + "=" + tpl.getCol()[j] + " ");
			}
			System.out.println();
		}
		displayPerformance();
	...
	public void displayPerformance() {
		System.out.println("Total " + getTuples().size()
	...
	public String toJSONString() {
		...
		catch (IOException e) {return "bad result string";}
		return out.toString();
	}
