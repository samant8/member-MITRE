...
public class ElasticSearchClient {
	...
	public void callServer(String query) {
		...
		try {
			while (true) {
				...
				JSONObject JRES = (JSONObject) jparser.parse(sb.toString()); sb=null;
	...
	public void getData(String sub, String pred, ArrayList<String> obj_lst) {
		...
		if(pred.equals("RegexTextSearch"))  {getData_fts(sub, pred, obj_lst); return;}

		if(!pred.equals(":")) obj = obj_lst.get(0); else obj = "";
		...
		if(!sub.startsWith("?")) {
			...
			if(pred.equals(":")) {getData_inst(obj_lst); return;}
			else {
